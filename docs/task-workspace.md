...
`TaskWorkspace` owns the shared landing geometry and non-conversation task entry. Before the first request, all four tasks use the centered task landing. After the first text message or audio transcription creates a conversation, Text or Audio must switch to Astryx's **AI Chat Conversation** composition (`ChatLayout` + `ChatMessageList`) instead of leaving the landing shell in place. Text keeps its message composer docked at the bottom. Audio docks the recording-file/transcribe composer in the same slot, renders the source audio as the user turn and the transcript as the assistant turn, and keeps newest turns nearest the composer. Long histories scroll inside the conversation. Reopening a saved Text or Audio chat enters conversation layout directly. Starting a new chat or switching tasks returns to the landing/task layout. Reference: https://astryx.atmeta.com/templates?preview=ai-chat.

Image and Embeddings retain the stable task input/results geometry. Audio shares that same landing geometry before the first transcription, then uses the same Astryx conversation shell as Text. The toolbar and model selector remain stable across task switches. Narrow screens keep the same stacked toolbar and input geometry; reduced-motion, focus and forced-colors behavior remain supported. Club artwork and semantic brand tokens are reused.
...
`FileDropzone.test.tsx` covers drop/browse parity, invalid input, duplicate operations, actual file delivery, legacy uploads and UTF-8 import. `UI-task-workspace.spec.ts` compares landing geometry across tasks at 1440, 390 and 320px in both themes, confirms displayed explanations, checks live-only exclusion, performs actual drop-to-model input flows, exercises the raw-vector and cosine-similarity embedding modes, verifies Whisper/Nova upload shapes through deterministic fixtures and retries readable failures without losing the recording. `UJ-004-browser-chat.spec.ts` verifies Text conversation behavior. Audio coverage verifies that the first transcription switches to the same ASTRYX conversation shell, keeps the audio source and transcript above the bottom composer, and reopens saved audio chats directly in conversation mode.
