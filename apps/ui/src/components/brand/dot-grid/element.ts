import { createFlareDots, FLARE_COLORS, FLARE_HEIGHT, FLARE_WIDTH, resetFlare, stepFlare, type FlareDot, type FlareStimulus } from './physics';

/** A small page-local canvas, not a document-wide animation engine.
 * Adapted from alexcodeplace/vibeflare d9471be (MIT); canonical public brand module.
 ...
 */
export function registerDotLogo(tagName = 'spargax-dot-logo'): void {
  if (typeof window === 'undefined' || !window.customElements || customElements.get(tagName)) return;
  ...
  connectedCallback(): void {
    ...
    const button = this.querySelector<HTMLButtonElement>('[data-dot-replay]');
    const hint = this.querySelector<HTMLElement>('[data-dot-hint]');
    if (!canvas || !button || !hint || !window.matchMedia || !window.IntersectionObserver || !window.ResizeObserver) return;
    ...
    const groups: FlareDot[][] = Array.from({ length: FLARE_COLORS.length + 1 }, () => []);
    ...
    const paint = () => {
      ...
      // Batch by palette: one fill operation per palette entry, no per-dot shadows or DOM nodes.
    ...
    const changePreferences = () => {
      ...
      hint.textContent = !motionAllowed() ? (this.dataset.hintRest ?? '') : finePointer.matches ? (this.dataset.hintPointer ?? '') : (this.dataset.hintTouch ?? '');
    ...
    const intersection = new IntersectionObserver(entries => {
      ...
      if (!entered && canRun()) replay(FLARE_WIDTH * .35, FLARE_HEIGHT * .45);
  ...
  customElements.define(tagName, FlareElement);
}
